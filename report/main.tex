...
react too fast to input changes. The relevant quantity is the true input Lipschitz
constant of the model, written with its explicit dependence on the weights $\wvec$,
\begin{equation}
\Lip_x\!\bigl(\yhat(\cdot\,;\wvec)\bigr)
=\sup_{x\neq x'}\frac{|\yhat(x;\wvec)-\yhat(x';\wvec)|}{|x-x'|}.
\label{eq:truelip}
\end{equation}
The ideal training problem is then
\begin{mini}
{\wvec}{f(\wvec)}{\label{eq:idealnlp}}{}
\addConstraint{\Lip_x\!\bigl(\yhat(\cdot\,;\wvec)\bigr)}{\le\Lmax}
\addConstraint{\norm{\wvec}_2^2}{\le\Bmax^2,}
\end{mini}
where $f(\wvec)$ is the training error \eqref{eq:mse}, the first constraint is the
desired Lipschitz requirement with bound $\Lmax$, and the second is an explicit
parameter-norm ball of radius $\Bmax$. The supremum in \eqref{eq:truelip} is not
directly convenient for a numerical NLP solver, so it is replaced next by a
conservative, computable sufficient condition.

\subsection{Derivation of the tractable certificate}
Because \eqref{eq:net} is differentiable, its Lipschitz constant is the supremum of
$|\mathrm d\yhat/\mathrm dx|$. Differentiating,
\begin{equation}
...
\begin{equation}
\Lip_x\!\bigl(\yhat(\cdot\,;\wvec)\bigr)\le\norm{\wvec_1}_2\,\norm{\wvec_2}_2=:\Uc .
\label{eq:cert}
\end{equation}
...
$\Lip_x(\yhat(\cdot\,;\wvec))\le\Lmax$. I impose it in the squared form
\begin{equation}
